/* eslint-disable no-underscore-dangle, prefer-destructuring, no-shadow, no-restricted-syntax, consistent-return, arrow-body-style, react/no-multi-comp */
/* copied from react-validation/src/hocs/form/index.js and react-validation/src/components/form/index.js */

import React, { PureComponent, Component } from 'react';
import PropTypes from 'prop-types';
import omit from 'lodash.omit';

const _isCheckable = component =>
  component.props.type === 'radio' || component.props.type === 'checkbox';
...
function form(WrappedComponent) {
  return class extends PureComponent {
      ...
      this.setState(
        {
          byId: {
            ...
            ...(isCheckable
              ? {
                  ...this.state.byName[this.state.byId[id].name].reduce(
                    (components, id) => {
                      components[id] = {
                        ...this.state.byId[id],
                        checked: false,
      ...
      this.setState(state => {
        return {
          byId: Object.keys(state.byId).reduce((byId, id) => {
            ...
            const components = Object.keys(state.byName).reduce(
              (byName, name) => {
    ...
    getValues = () =>
      Object.keys(this.state.byName).reduce((values, name) => {
        if (this.state.byName[name].length > 1) {
          values[name] = this.state.byName[name].map(
            id => this.state.byId[id].value,
          );
        } else {
