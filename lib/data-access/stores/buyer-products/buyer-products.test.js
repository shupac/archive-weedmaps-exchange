...
describe('BuyerProducts Store', () => {
  ...
  it('can fetch featured products and set data', async () => {
    const mockFetchClient = {
      fetch: jest.fn().mockReturnValue({ data: mockCategoryProducts }),
    };
    const buyerProducts = BuyerProducts.create(
    ...
    await buyerProducts.getFeaturedProducts();
    expect(mockFetchClient.fetch).toHaveBeenCalledWith(
      '/buyer/products/featured',
    );
    expect(getSnapshot(buyerProducts.featuredProductsData)).toMatchSnapshot();
  });
});
