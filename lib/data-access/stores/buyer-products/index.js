...
const BuyerProducts = types
  ...
  .views(self => ({
    ...
    get productDetails() {
      ...
      return {
        ...
        avatarImage: get(self.productDetailsData, 'product.avatarImage'),
        galleryImages,
      };
    },
    get productVariants() {
  ...
  .actions(self => ({
    ...
    async getFeaturedProducts() {
      self.setFeaturedProductsLoading(true);
      try {
        const { data } = await getEnv(self).client.fetch(
          '/buyer/products/featured',
        );
        self.setFeaturedProductsData(data);
      } catch (e) {
        logger.error(e);
      } finally {
        self.setFeaturedProductsLoading(false);
      }
    },
    ...
    setFeaturedProductsLoading(loading) {
      self.featuredProductsLoading = loading;
    },
  }));
...
export type BuyerProductsType = {
  ...
  setFeaturedProductsLoading: boolean => void,
};
