...
import Brand, { type BrandType, type BrandInputType } from 'models/brand';
...
const AuthStore = types
  ...
  .actions(self => ({
    ...
    async updateOrganization(organization) {
      try {
        const { id, licenses } = organization;
        ...
        const mappedLicenses = licenses.map(
          // eslint-disable-next-line
          ({ licenseType: license_type, ...rest }) => ({
            license_type,
            ...rest,
          }),
        );
        ...
        mappedLocation.licenses = mappedLicenses;
    ...
    async updateBrand(brand) {
      const payload = {
        data: {
          type: 'brand',
          attributes: {
            ...formatAttributes(brand, 'snakeCase'),
          },
        },
      };
      try {
...
export type AuthStoreType = {
  ...
  updateBrand: BrandInputType => Promise<boolean>,
};
