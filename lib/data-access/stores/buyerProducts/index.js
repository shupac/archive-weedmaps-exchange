...
const BuyerProducts = types
  ...
  .views(self => ({
    ...
    get productDetails() {
      const galleryImages = get(self.product, 'product.galleryImages', []).map(
      ...
      const variants = get(self.product, 'variants', []).map(
        ({ unit, name, id, defaultPrice, size }) => ({
          id,
          name,
          size,
          defaultPrice,
      ...
      return {
        ...
        brand: get(self.productDetailsData, 'brand.name', ''),
