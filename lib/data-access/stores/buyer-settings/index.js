...
const BuyerSettings = types
  ...
  .actions(self => ({
    ...
    async updateActiveLocation(locationId) {
      const currentActiveLocationId = self.activeLocation.id;
      await self.getLocations();
      const nextLocation = self.locations.find(
        location => location.id === locationId,
      );
      try {
        // Exit if selecting a location that is already active exit
        if (nextLocation.active) return true;
        // Here we mutate our observable directly
        self.setActiveLocation(locationId);
        return true;
      } catch (e) {
        logger.error(e);
        self.setActiveLocation(currentActiveLocationId);
        return false;
      }
    },
...
export type BuyerSettingsType = {
  ...
  updateActiveLocation: string => Promise<boolean>,
