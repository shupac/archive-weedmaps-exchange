...
const Zones = types
  ...
  .actions(self => ({
    ...
    fetchRegionsForZones: flow(function* fetchRegionsForZones() {
      const promises = [];
      // TODO remove california slug filter hack when we have a better approach
          ...
          getEnv(self)
            .wmSdk.fetch(
              `${config.apiGatewayUrl}/discovery/v1/regions/${
                region.wmRegionId
              }?include[]=geometry&filter[exclude_slugs][]=california`,
              { timeout: 7000 },
            )
    ...
    fetchRegionsInBounds: flow(function* fetchRegionsInBounds(
      ...
      const {
        ...
        .wmSdk.fetch(
          ...
          }/discovery/v1/regions?include[]=geometry&filter[exclude_slugs][]=california&filter[bounding_box]=${bbox}&filter[published_for][]=brands&limit=25`,
          { timeout: 7000 },
        )
        .then(res => res && res.json());
