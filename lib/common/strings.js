...
export function isPriceInvalid(price) {
  return !price || !Number.isFinite(parseFloat(price));
}
...
export function stripNonNumbers(rawString, allowDecimal) {
  if (allowDecimal) return rawString.replace(/[^\d.]/g, '');
...
export const concatAddy = (name, address) =>
  `${name} - ${address.streetAddress}, ${address.city}, ${address.territory}`;

export const formatOrderId = orderId => orderId.substr(0, 6).toUpperCase();

export const formatCurrency = (value, options = {}) => {
  const { addCommas = false, round, prefix = '' } = options;
  value = value.toString().replace(/^\$|,/g, '');
  if (parseFloat(value) === 0) return `${prefix}0`;
  ...
  if (addCommas) value = value.replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,');
  return `${prefix}${value}`;
};
...
export const getPaginationText = (
  ...
  return `Showing ${start} - ${end} of ${totalItemsCount} ${label}`;
};
...
export const toCurrency = value => {
  ...
  const digits = value.replace(/^0+|(-(?!\d))|[^0-9|-]/g, '') || '';

  // pad digits
  const desiredLength = 3;
  const actualLength = digits.length;
  if (actualLength >= desiredLength) {
    digitsWithPadding = digits;
  } else {
    const amountToAdd = desiredLength - actualLength;
    const padding = '0'.repeat(amountToAdd);
    digitsWithPadding = padding + digits;
  }

  // add decimal to number
  const centsStartingPosition = digitsWithPadding.length - 2;
  const dollars = digitsWithPadding.substring(0, centsStartingPosition);
  const cents = digitsWithPadding.substring(centsStartingPosition);
