...
describe('Strings', () => {
  ...
  describe('formatCurrency', () => {
    it('should add dollar sign, insert commas, and add trailing zeros', () => {
      expect(formatCurrency('10')).toEqual('10.00');
      expect(
        formatCurrency('10,000', { prefix: '$', addCommas: true }),
      ).toEqual('$10,000.00');
    });
  });

  describe('getPaginationText', () => {
    it('return a string indicating start and end', () => {
      const paginationText = getPaginationText(85, 25, 4, 'Orders');
      expect(paginationText).toEqual('Showing 76 - 85 of 85 Orders');
    });
  });
});
